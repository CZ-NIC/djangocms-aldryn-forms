"""
This is almost same code as in cms/forms/utils.py. But this code supports djangocms-versioning.
"""
...
from cms.cache.choices import _page_cache_key, _site_cache_key
...
def get_page_choices_for_site(site, language):
    ...
    translation_lookup = Prefetch(
        ...
        queryset=PageContent.admin_manager.filter(language__in=languages).only('pk', 'page', 'language', 'title')
    )
    pages = (
        Page
        .objects
